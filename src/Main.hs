...
type family Elem (e :: PackageName) (es :: [PackageName]) ::
     Constraint where
        Elem x (x ': xs) = ()
        Elem y (x ': xs) = Elem y xs

class HasImportInstructions (p :: PackageName)  where
    requirePackage :: Proxy (p :: PackageName) -> IO ()
    packageName :: Proxy (p :: PackageName) -> String
...
runRequirePackage
    :: (Elem p ps, HasImportInstructions p)
    => Proxy (p :: PackageName) -> Proxy (ps :: [PackageName]) -> IO ()
